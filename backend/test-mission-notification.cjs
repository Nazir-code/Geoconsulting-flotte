...
// Initialiser Firebase Admin directement
async function initializeFirebase() {
  try {
    // Vérifier les variables d'environnement
    if (!process.env.FIREBASE_PROJECT_ID) {
      throw new Error('FIREBASE_PROJECT_ID manquant dans les variables d\'environnement');
    }
...
async function testMissionNotification() {
  try {
    ...
    console.log('🔧 Pour obtenir le token FCM depuis Flutter:');
